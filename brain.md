...
## Sprint #40: Hindi — 6th language, full 691-key dictionary, meteor fix, chrome audit

Urdu/Arabic/Spanish/Chinese added #25–#36; this sprint ships **हिन्दी** in one self-contained pass. Dictionary now has a typed `hi: Dict` block (691/691 leaf keys exact parity with English — `dictionaries.test.ts` enforces identical leaf-key tree + every leaf non-empty), `Locale` union + LOCALES entry `{ code: "hi", label: "हिन्दी", dir: "ltr", htmlLang: "hi" }`, registry order `{ en, ur, ar, es, zh, hi }`, and the `layout.tsx` prerender inline script gained `else if(l==="hi"){lang="hi";}`. No server-side locale reads exist — translation is client-driven (cookie `zunara-locale` → `LocaleProvider`); the inline script only flips `<html lang/dir>` pre-hydration.

**How the translation was authored/landed (repeatable):** read all 691 English leaf values for context, hand-authored Hindi in 3 flat `.cjs` chunk files, then a single generator script (`Temp\opencode\insert-hi.cjs`): merged chunks, asserted key-set equality against `en-leaves.json` (missing + extra), rejected empty values, enforced placeholder-superset (every `{…}` token in Hindi must already exist in the English string — e.g. `changes.formsAspect` legally omits `{article}`), built the nested object, serialized in the dict's existing 2-space style (quoting non-identifier keys like `"Actor & director"`), and spliced union + LOCALES + block + registry into `dictionaries.ts`. Terminology locked: signs मेष…मीन, elements अग्नि/पृथ्वी/वायु/जल, modalities चर/स्थिर/द्विस्वभाव, planets incl. राहु (उत्तर नोड)/केतु (दक्षिण नोड), retrograde = प्रतिगामी (NOT वक्री), conj=युति / opp=विरोध / sext=षट्कोण / square=वर्ग / trine=त्रिकोण, brand "ज़ुनारा", acronyms (VSOP87/UTC/RTL) kept as-is, "कल" ambiguity split as बीता कल/आने वाला कल.

**Meteor fix (the one nobody expected):** the `<canvas>` sky vanished on phones because `meteor-shower.tsx` returned early under `prefers-reduced-motion` — a near-universal mobile default — leaving a blank void. First pass painted a DETERMINISTIC static tableau (faint star field + frozen streaks, zero rAF). The user's phone still read that as broken ("meteors are not moving"), so the reduce branch was reworked into a **gentle animated mode**: it always animates, but caps concurrent streaks to 2, spawns slower (×0.55 speed, first pattern at 1.2s), fades streaks softer (alpha ×0.55, duration ×1.6), skips the hero flare/clusters, leaves no smoke residue, and re-paints the procedural star field every frame via `paintStars()` after `clearRect`. Normal mode thickens streaks slightly on small screens (1.25–1.8px vs 0.9–1.5px desktop). Result: meteors visibly drift on every device, including phones with reduced-motion enabled.

**Round 2 (translation + mobile, PR #40b):** per the user's follow-up — "some words still in English", "mobile unusable", "meteors don't move".
- **`uichrome` chrome section** (18 keys) added to all 6 locales via a generator script (`Temp\opencode\add-uichrome.cjs`): inPlainWords/openFeature/theZodiac, place pick/verified/manual messages (chart + map variants), placeExample sample with `{city}/{country}` tokens, steadyDay, didYouKnow, errorReload, positionUpdatesLive, nodeNorth/nodeSouth, todaysHoroscope + readTodays (with `{sign}` token). Parity still enforced by `dictionaries.test.ts`.
- **Date/month localization without dict keys:** new `src/lib/i18n/date.ts` (BCP47 map + `monthLong`/`monthShort`/`formatDate`, UTC+try/catch) and client `<LocaleDate>` (`src/components/ui/locale-date.tsx`). Wired into: home kicker date, "On this day" heading, `People born on …` heading, birthchart "Computed for {date}", birth-form month `<option>`s, trend-timeline month range. Wiki URL slug keeps English month names.
- **Unlocalized bug fixes:** retrograde table called `tSign(signName)` with an English **name** (tSign wants a slug) — now `tSign(signDot.sign)`; daily-transit card rendered Body English `meta.name` → `tPlanet`; famous-birthdays sign-of-the-day name → `tSign(sign.slug)`; lunar-node pill ("Position updates live"/North-South Node + live degree) fully localized.
- **Remaining chrome:** home page ("In plain words:" ×4, "Open", "the zodiac" fallback), lunar/place hint strings, daily-orbit aria-labels, error-boundary fallback, "Did you know?", "A steady day" headline fallback, birth-place placeholder — all routed through `uichrome`/`LocaleText`. Server-rendered editorial copy stays English by design (recorded Sprint #40a scope note).
- **Mobile P0 fixes:** `period-tabs.tsx` nav now `max-w-full overflow-x-auto whitespace-nowrap` (4 tabs no longer clipped by `overflow-x: clip` on html); `chart-wheel.tsx` svg wrapper `relative w-full` + `style={{maxWidth:size}}` + `h-auto w-full` (wheel scales to any viewport); cosmic-facts sign grid `grid-cols-2 sm:grid-cols-3 lg:grid-cols-6`; big-three/age-header grids `gap-2 sm:gap-3` at 3 cols (no wrap at 320px).
- **Verification:** 377/377 tests, tsc 0, eslint 0 errors (only pre-existing exhaustive-deps warnings), `next build` green, SSR GET / = 200, hi/zh uichrome strings confirmed in built client chunks. Pushed with this commit.

**Push hash update:** Sprint #40 (Hindi) = `7e28a81`; follow-up round 2 = see commit below.
...
Fix (decision: one scalable free, API-key-free, date-keyed source — enwiki day-articles — is the right backbone; evaluated MusicBrainz/DBpedia/TVMaze as alternates and rejected each: MusicBrainz `begin:` is an ABSOLUTE date interval with no MM-DD matching per year, DBpedia SPARQL unreachable from the sandbox, TVMaze is name-only search):
- Gap audit (fixed per-line parse of all 3k pool entries): all 365 dates ≥6 entries, but **105 dates had <3 non-sports entries**.
- Re-baked `rebalance-main.mjs`: for those 105 dates, classified the 25k births-cache into industry buckets (entertainment/music/literature/science/leadership/arts/business/other), picked ≤5/date bucket round-robin, excluded sports + existing primary/pool names → **525 new diverse entries**, portraits via enwiki `pageimages&pithumbsize=330` (419 with images, 106 wiki-only), injected as `_poolExtra` in `celebrity-pool.ts`. Pool now **3,271 entries**; **0 dates <3 non-sports**, sports share 55% → **46%**.
- `diversitySelect` (celebrities.ts) is now industry-aware: fill loop skips athletes once `selected.filter(isAthlete).length >= 2` (region pass already reserves one Sports slot → visible sports ≤3 of 6, non-sports ≥3, guaranteed). Exported `isAthlete` (regex over profession/notes).
- Tests: replaced "≥3 people" with **"≥3 non-athletes per date"** + new **"≤3 athletes per date"** sweep over all 365 dates — 371/371 green, tsc 0, lint 0 errors, build OK, SSR verified (today's home shows rapper + footballer + engineer etc., sports in minority).
- Source-stack honesty: Wikipedia day-article cache (full industry mining) is the static backbone; the LIVE tier still tops up via Wikidata SPARQL in production — the system already spans multiple free sources; the overload was a *selection* bug, not a missing-source bug.
