...
**Meteor fix (the one nobody expected):** the `<canvas>` sky vanished on phones because `meteor-shower.tsx` returned early under `prefers-reduced-motion` — a near-universal mobile default — leaving a blank void. Now the reduce branch paints a single DETERMINISTIC static tableau (faint star field + 2-3 frozen streaks, re-drawn on resize, zero rAF cost) and normal mode thickens streaks slightly on small screens (1.25–1.8px vs 0.9–1.5px desktop) where 1px shavings are invisible.
