    ...
    <div className="constellation-bg">
      <section className="relative overflow-hidden border-b border-line-soft">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 pb-14 pt-16 text-center sm:px-6 sm:pt-24">
          <p className="kicker">The cosmic vault</p>
          ...
          <p className="mx-auto mt-6 max-w-xl text-lg leading-8 text-muted">
            Explore every zodiac sign&rsquo;s core traits, secret perks, ruling planet lore,
            and the elemental forces that shape each personality.
          </p>
        </div>
      </section>

      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <p className="kicker text-center">Choose your sign</p>
        <h2 className="mt-3 text-center font-display text-2xl text-starlight">
          Tap a sign to reveal its cosmic profile
        </h2>
            ...
            return (
              ...
              >
                <span className={`transition-transform duration-300 ${active ? "scale-110" : "group-hover:scale-105"}`}>
                  <ZodiacSymbol sign={sign.slug} size="lg" className={active ? "text-gold" : elementText(sign.element)} label={sign.name} />
                </span>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-20 sm:px-6">
        <div className="paper-panel relative overflow-hidden p-8 sm:p-10">
          ...
          <div className="relative">
            <p className="kicker">Elemental forces</p>
            <h2 className="mt-3 font-display text-3xl text-starlight sm:text-4xl">
              The four pillars of the zodiac
            </h2>
            <p className="mt-3 max-w-xl leading-7 text-muted">
              Every sign draws its fundamental energy from one of four elements. Understanding your
              element reveals the deeper current beneath your personality.
            </p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className={`font-display text-xl font-semibold ${data.color}`}>{el}</h3>
                    </div>
                    <p className="mt-3 text-sm leading-6 text-muted">{data.desc}</p>
                    <ul className="mt-4 space-y-1.5">
                      {data.signs.map((name) => (
                        <li key={name} className="text-sm text-p-ink">{name}</li>
                      ))}
                    </ul>
                  </div>
                );
...
function SignProfile({ sign, onClose }: { sign: ZodiacSign; onClose: () => void }) {
  ...
  const sameElement = ZODIAC_SIGNS.filter((s) => s.element === sign.element && s.slug !== sign.slug).map((s) => s.name);

  return (
    <section className="mx-auto max-w-3xl px-4 pb-16 sm:px-6" aria-label={`${sign.name} cosmic profile`}>
        ...
        <div className="relative p-8 sm:p-10">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-5">
              <div className="flex h-16 w-16 items-center justify-center rounded-full border border-line bg-ink-2">
          ...
          <div className="mt-8">
            <h3 className="kicker">Key traits &amp; personality</h3>
            <div className="mt-4 flex flex-wrap gap-2">
              {sign.traits.map((t) => (
                <span key={t} className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-sm text-p-ink">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2">
            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
              ...
              <ul className="mt-3 space-y-2">
                {sign.traits.slice(0, 4).map((t) => (
                  <li key={t} className="flex items-center gap-2 text-sm text-p-ink">
          ...
          <div className="mt-8 rounded-xl border border-white/10 bg-white/[0.03] p-5">
            <h3 className="kicker">Compatibility &amp; element power</h3>
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-xs font-semibold text-p-muted uppercase tracking-wide">Same element</p>
                <p className="mt-1 text-sm leading-6 text-p-ink">{compat.same}</p>
              </div>
              <div className="border-t border-p-line pt-4">
                <p className="text-xs font-semibold text-p-muted uppercase tracking-wide">Best pairing</p>
                <p className="mt-1 text-sm leading-6 text-p-ink">{compat.best}</p>
              </div>
              <div className="border-t border-p-line pt-4">
                <p className="text-xs font-semibold text-p-muted uppercase tracking-wide">Challenging pairing</p>
            ...
            {sameElement.length > 0 && (
              <div className="mt-5 border-t border-p-line pt-4">
                ...
                <p className="mt-1 text-sm text-p-ink">{sameElement.join(" · ")}</p>
              </div>
            )}
          </div>
        </div>
