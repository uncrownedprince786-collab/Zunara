...
const GLOW: Record<Element, string> = {
  ...
  Earth: "bg-[radial-gradient(circle,rgba(16,185,129,0.28)_0%,transparent_70%)]",
    ...
    <div className="flex-1">
        ...
        {ZODIAC_SIGNS.map((sign) => {
          ...
          return (
            ...
            >
              ...
              <span className="hidden text-[0.7rem] font-medium sm:inline">{sign.name}</span>
            </button>
          );
        })}
      </div>
      {current && (
        <p className="mt-3 text-xs text-subdued">
          {current.name} · {current.element} · Ruled by {current.ruler}
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="paper-panel relative overflow-hidden p-8 sm:p-10">
      ...
      <div className="relative grid gap-8 lg:grid-cols-2">
        <SignPicker label="Select sign one" value={a} onChange={setA} />
        <SignPicker label="Select sign two" value={b} onChange={setB} />
      </div>
      ...
      {result ? (
          ...
          {signA && signB && (
            <div className="flex items-center justify-center gap-4">
              <span className="text-gold">
                <ZodiacSymbol sign={signA.slug} size="lg" label={signA.name} />
              </span>
              <span aria-hidden className="font-display text-2xl text-muted">&amp;</span>
              <span className="text-gold">
                <ZodiacSymbol sign={signB.slug} size="lg" label={signB.name} />
              </span>
              <span className="ml-2 font-display text-3xl font-semibold text-starlight">
                {result.overall}%
              </span>
            </div>
          )}
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-3">
            {result.pillars.map((p) => (
              <div key={p.label} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <ScoreBar label={p.label} score={p.score} tint={tint} />
        ...
        <p className="relative mt-10 text-sm text-muted">Choose two signs to reveal how they mesh.</p>
      )}
    </div>
  );
}
